...
{
    coro_t *coro = (coro_t *)malloc(sizeof(*coro) + CORO_STACK_MIN);
    if (!coro)
        return NULL;
...
{
    coro_defer_t *defer = (coro_defer_t *)malloc(sizeof(*defer));
    if (UNLIKELY(!defer))
        return;
...
{
    coro_defer_t *defer = (coro_defer_t *)malloc(sizeof(*defer) + size);
    if (UNLIKELY(!defer))
        return NULL;
...
coro_strdup(coro_t *coro, const char *str)
{
    ...
    char *dup = (char *)coro_malloc(coro, len);
    if (LIKELY(dup))
        memcpy(dup, str, len);
    return dup;
}
